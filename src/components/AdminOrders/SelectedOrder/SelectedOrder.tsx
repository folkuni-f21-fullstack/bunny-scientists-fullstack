...
type Props = {
  ...
  allOrders: Order[];
  setOriginalAllOrders: (
    value: Order[] | ((prevVar: Order[]) => Order[])
  ) => void;
};
...
const SelectedOrder = ({
  ...
  useEffect(() => {
    ...
    let newFilteredMenu: MenuItem[] = menuList.filter((dish) => {
      const result = selectedOrderItems.find(
        (compare) => dish.id === compare.id
      );
      return !result;
    });
    setMessageToChef("");
    ...
    dishCopy.map((dish) => {
      ...
      if (dish.amount < 1 && dish.menuItem.id === order.menuItem.id) {
        ...
        newFilteredMenu = menuList.filter((dish) => {
          const result = menuItemArray.find(
            (compare) => dish.id === compare.id
          );
          return !result;
        });
  ...
  async function addToArchive() {
    ...
    await fetch(`/api/orders/${archiveObj.orderNumber}`, {
      method: "DELETE",
    });
      ...
      <div className="confirm-container">
        {allOrders.length < 1 ? ( // * Visa grå button om det inte finns några ordrar
          ...
          <>
            {selectedOrder ? (
              <>
                {JSON.stringify(selectedOrder.orderItems) !==
                JSON.stringify(selectedOrderItem) ? ( // * Visa extra text om ordern har ändrats
                  <>
                    <button
                    ...
                    <p className="red">
                      <strong>!!</strong> Dina ändringar{" "}
                      <strong>sparas ej</strong> om du byter order <br /> Snälla{" "}
                      <strong>Bekräfta</strong> ordern{" "}
                    </p>
                  </>
                ) : (
                  <button
